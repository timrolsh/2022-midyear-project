...
def draw_destination_card_screen(screen, current_turn):
    ...
    def draw_table(ax, ay, width, height, size):

        anew_ranges = []
        current_card = 0
        for y_val in range(ay, ay + size[1], height):
            row_rect = 1
            for x_val in range(ax, ax + size[0], width):
                if row_rect == 1:
            ...
            anew_ranges.append([player.destination_cards[current_card], ax, ax + width, y_val, y_val + height])
            anew_ranges.append([player.destination_cards[current_card], ax, ax + width * 2, y_val, y_val + height])
            anew_ranges.append([player.destination_cards[current_card], ax, ax + width * 3, y_val, y_val + height])
            current_card += 1
        return anew_ranges
