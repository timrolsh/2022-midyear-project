...
class Player:
    ...
    def claim_tracks(self, track: Track):
        """
        Claim a track using the train cards a player currently has. This only works if the cards are the same color as the track, and the player has enough cards.
        """
        if track.occupied:
    ...
    def claim_destination(self):
        """
        Claim a destination card and add points to the user's score. Uses the UnionFind algorithm to check if two cities are connected. 
        """
